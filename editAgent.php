	...
	<form action="editAgent.php" method="post" id="formGeneral">
		...
		if ($send == 1) {
			if ($num_rows > 0 && $username != $oldUsername) {
				echo "<label style=color:var(--error-forecolor)>" . $translations["AGENT_ALREADY_EXIST"] . "</label><br><br>";
			} else {
				echo "<label style=color:var(--success-forecolor)>" . $translations["SUCCESS_UPDATE_AGENT_DATA"] . "</label><br><br>";
			}
		}
		?>
