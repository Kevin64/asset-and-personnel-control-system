...
if ($total >= 1) {
	...
	$query = mysqli_query($conexao, "insert into patrimonio (patrimonio, descarte, lacre, sala, predio, ad, padrao, dataFormatacao, marca, modelo, numSerie, processador, memoria, hd, sistemaOperacional, hostname, bios, mac, ip, emUso, etiqueta, tipo, tipoFW, tipoArmaz, gpu, modoArmaz, secBoot, vt, tpm, trocaPilha, ticketNum) values('$patrimonio', 0, '$numeroLacre', '$sala', '$predio', '$ad', '$padrao', '$dataFormatacao', '$marca', '$modelo', '$numeroSerial', '$processador', '$memoria', '$hd', '$sistemaOperacional', '$nomeDoComputador', '$bios', '$mac', '$ip', '$emUso', '$etiqueta', '$tipo', '$tipoFW', '$tipoArmaz', '$gpu', '$modoArmaz', '$secBoot', '$vt', '$tpm', '$trocaPilha', '$ticketNum');") or die($translations["ERROR_ADD_DATA"] . mysqli_error($conexao));
	
	$queryFormatAnt = mysqli_query($conexao, "insert into manutencoes (patrimonioFK, dataFormatacoesAnteriores, modoServico, trocaPilha, ticketNum, agent) values('$patrimonio', '$dataFormatacaoExpandida', '$modoServico', '$trocaPilha', '$ticketNum', '$agent');") or die($translations["ERROR_ADD_DATA"] . mysqli_error($conexao));
	$mensagem = $translations["NEW_ASSET_REGISTERING_DATA"];
}
