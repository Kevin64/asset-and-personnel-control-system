...
ADD COLUMN `name` VARCHAR(45) NULL DEFAULT NULL
AFTER `password`,
  ADD COLUMN `surname` VARCHAR(45) NULL DEFAULT NULL
AFTER `name`;
ALTER TABLE `apcsdb`.`apcs_asset`
ADD COLUMN `assetHash` VARCHAR(64) NULL DEFAULT NULL
AFTER `note`,
  ADD COLUMN `hwHash` VARCHAR(64) NULL DEFAULT NULL
AFTER `assethash`;
ALTER TABLE `apcsdb`.`apcs_asset_maintenances` CHANGE COLUMN `previousServiceDates` `mainServiceDate` VARCHAR(10) NULL DEFAULT NULL,
...
#--------------------------------------------------------------------------------------------#
insert into apcsdb.apcs_asset_hardware (
    `assetNumberFK`,
    `hwBrand`,
    `hwModel`,
    `hwType`,
...
select `assetNumber`,
  ...
  `hwType`,
  `serialNumber`
from apcsdb.apcs_asset;
#--------------------------------------------------------------------------------------------#
insert into apcsdb.apcs_asset_firmware (
    `assetNumberFK`,
    `fwType`,
    `fwVersion`,
    `fwMediaOperationMode`,
    `fwSecureBoot`,
    `fwVirtualizationTechnology`,
    `fwTpmVersion`
  )
select `assetNumber`,
  `fwType`,
...
from apcsdb.apcs_asset;
#--------------------------------------------------------------------------------------------#
insert into apcsdb.apcs_asset_location (
    `assetNumberFK`,
    `locBuilding`,
    `locRoomNumber`,
    `locDeliveredToRegistrationNumber`,
    `locLastDeliveryMadeBy`,
    `locLastDeliveryDate`
  )
select `assetNumber`,
  `building`,
  `roomNumber`,
  `deliveredToRegistrationNumber`,
  `lastDeliveryMadeBy`,
  `lastDeliveryDate`
from apcsdb.apcs_asset;
#--------------------------------------------------------------------------------------------#
...
#--------------------------------------------------------------------------------------------#
insert into apcsdb.apcs_asset_network (
    `assetNumberFK`,
    `netMacAddress`,
    `netIpAddress`,
    `netHostname`
...
SELECT `assetNumber`,
  SUBSTRING_INDEX(`operatingSystem`, ',', 1),
  substring_index(
    substring_index(
      SUBSTRING_INDEX(`operatingSystem`, ',', 3),
      'build ',
      -1
    ),
    ' ',
  ...
  substring_index(
    SUBSTRING_INDEX(`operatingSystem`, ',', 2),
    'v',
    -1
  ),
  '0'
FROM apcs_asset
WHERE operatingSystem LIKE '%32 bits';
insert IGNORE into apcsdb.apcs_asset_operating_system (
    `assetNumberFK`,
    `osName`,
    `osBuild`,
...
SELECT `assetNumber`,
  ...
  '1'
FROM apcs_asset
WHERE operatingSystem LIKE '%64 bits';
insert IGNORE into apcsdb.apcs_asset_operating_system (
    `assetNumberFK`,
    `osName`,
    `osBuild`,
...
SELECT `assetNumber`,
  ...
  '1'
FROM apcs_asset
WHERE operatingSystem LIKE '%64)';
#--------------------------------------------------------------------------------------------#
insert IGNORE into apcsdb.apcs_asset_processor (
    `assetNumberFK`,
    `procId`,
    `procName`,
    `procFrequency`,
...
SELECT `assetNumber`,
  ...
  `processor`,
  cast(
    substring_index(SUBSTRING_INDEX(`processor`, ' MHz', 1), ' ', -1) as unsigned
  ),
  cast(
    substring_index(SUBSTRING_INDEX(`processor`, 'C/', 1), '(', -1) as unsigned
  ),
  cast(
    substring_index(SUBSTRING_INDEX(`processor`, 'T)', 1), '/', -1) as unsigned
  )
FROM `apcsdb`.`apcs_asset`;
#--------------------------------------------------------------------------------------------#
insert IGNORE into apcsdb.apcs_asset_ram (
    `assetNumberFK`,
    `ramAmount`,
    `ramType`,
    `ramFrequency`,
...
select `assetNumber`,
  ...
  '24',
  substring_index(substring_index(`ram`, 'MHz', 1), ' ', -1),
  '0'
from apcsdb.apcs_asset
where `ram` like '%DDR3%';
insert IGNORE into apcsdb.apcs_asset_ram (
...
from apcsdb.apcs_asset
where `ram` like '%DDR4%';
update apcsdb.apcs_asset_ram
set ramAmount = ramAmount * 1073741824;
#--------------------------------------------------------------------------------------------#
insert IGNORE into apcsdb.apcs_asset_video_card (`assetNumberFK`, `vcName`, `vcId`, `vcRam`)
SELECT `assetNumber`,
  `videoCard`,
  '0',
  substring_index(
    substring_index(SUBSTRING_INDEX(`videoCard`, '%B)', 1), '(', -1),
    ' ',
    1
  )
FROM `apcsdb`.`apcs_asset`
WHERE videoCard LIKE '%GB)';
update apcsdb.apcs_asset_video_card
set vcRam = vcRam * 1073741824
WHERE `vcName` LIKE '%GB)';
insert IGNORE into apcsdb.apcs_asset_video_card (`assetNumberFK`, `vcName`, `vcId`, `vcRam`)
SELECT `assetNumber`,
  `videoCard`,
  '0',
...
FROM `apcsdb`.`apcs_asset`
WHERE videoCard LIKE '%MB)';
update apcsdb.apcs_asset_video_card
set vcRam = vcRam * 1048576
WHERE `vcName` LIKE '%MB)';
#--------------------------------------------------------------------------------------------#
insert IGNORE into apcsdb.apcs_asset_storage (
    `assetNumberFK`,
    `storSize`,
    `storSmartStatus`,
    `storId`
...
select `assetNumber`,
  replace(substring_index(`storageSize`, ' ', 1), ',', '.') * 1000000000000.0,
  '0',
  '0'
from apcsdb.apcs_asset
WHERE `storageSize` like '%,%';
insert IGNORE into apcsdb.apcs_asset_storage (
    `assetNumberFK`,
    `storSize`,
    `storSmartStatus`,
...
select `assetNumber`,
  replace(substring_index(`storageSize`, ' ', 1), ',', '.') * 1000000000,
  '0',
  '0'
from apcsdb.apcs_asset
WHERE `storageSize` not like '%,%'
  and `storageSize` not like '%1 %'
  and `storageSize` not like '%2 %'
  and `storageSize` not like '%3 %'
  and `storageSize` not like '%4 %';
SET SQL_SAFE_UPDATES = 1;
ALTER TABLE `apcsdb`.`apcs_asset` DROP COLUMN `tpmVersion`,
