...
if ($total >= 1) {
	...
	$queryFormatPrevious = mysqli_query($connection, "insert into " . $dbMaintenancesArray["MAINTENANCES_TABLE"] . " (" . $dbMaintenancesArray["ASSET_NUMBER_FK"] . ", " . $dbMaintenancesArray["PREVIOUS_SERVICE_DATES"] . ", " . $dbMaintenancesArray["SERVICE_TYPE"] . ", " . $dbMaintenancesArray["BATTERY_CHANGE"] . ", " . $dbMaintenancesArray["TICKET_NUMBER"] . ", " . $dbMaintenancesArray["AGENT_ID"] . ") values('$assetNumber', '$serviceDate', '$serviceType', '$batteryChange', '$ticketNumber', '$agent');") or die($translations["ERORR_ADD_DATA"] . mysqli_error($connection));
